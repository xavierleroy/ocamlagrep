...
val pattern: ?transl:string -> string -> pattern
        (* Compile a search pattern.  The syntax for patterns is
         ...
         - [&    ]  conjunction (e.g. [Roux&Combaluzier])
         - [|    ]  alternative (e.g. [fromage|dessert])
         - [(..) ]  grouping
...
val pattern_string: ?transl:string -> string -> pattern
        (* [Agrep.pattern_string s] returns a pattern that matches exactly
           the string [s] and nothing else.  The optional argument
...
module Iso8859_1: sig
  ...
  (* Useful translation tables for the ISO 8859-1 (Latin-1) character set. *)
